...
TFrac<T>::TFrac(string frac)
{
	...
	if (numbers.size() != 2) {
		throw FracException("Must be two nubers!");
	}
...
void TFrac<T>::init(T a, T b)
{
	if (is_same<T, float>::value || is_same<T, double>::value) {
		throw FracException("Incorrect type!");
	}

	if (b == 0) {
		throw FracException("B equals 0!");
	}
...
class FracException : public exception
{
public:
	FracException(const char* s) : exception(s)
	{
	}
};